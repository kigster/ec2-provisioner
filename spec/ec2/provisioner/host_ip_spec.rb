...
describe EC2::Provisioner::HostIP do
  describe '.ip_for' do
    ...
    it 'returns the IP address given a hostname' do
      expect(EC2::Provisioner::HostIP.ip_for('app001.stage')).to eq('1.1.1.1')
      expect(EC2::Provisioner::HostIP.ip_for('app002.stage')).to eq('1.1.1.2')
    end

    context 'server has multiple IPs' do
      it 'returns the first IP' do
        expect(EC2::Provisioner::HostIP.ip_for('app003.stage')).to eq('1.1.1.3')
      end
    end
  end
end
